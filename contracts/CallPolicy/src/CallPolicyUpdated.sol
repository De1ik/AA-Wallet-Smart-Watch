...
contract CallPolicy is PolicyBase {
        ...
        returns (uint256)
    {
        ...
        if (callType == CallType.CALLTYPE_SINGLE) {
        ...
        } else if (callType == CallType.CALLTYPE_BATCH) {
            Execution[] calldata exec = ExecLib.decodeBatch(executionCallData);
            for (uint256 i = 0; i < exec.length; i++) {
                if (!_checkPermission(msg.sender, id, CallType.CALLTYPE_SINGLE, exec[i].target, exec[i].callData, exec[i].value)) {
                    revert CallViolatesParamRule();
                }
            }
        } else if (callType == CallType.CALLTYPE_DELEGATECALL) {
